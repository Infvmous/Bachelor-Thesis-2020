var adminObject = {
	...
	clickRemoveRowTemplate: function (id, span, url) {
		...
		thisTemp += '" class="clickRemoveRow mrr5">Да</a>';
		thisTemp += '<a href="#" class="clickRemoveRowConfirm">Нет</a>';
		thisTemp += '</div>';
		thisTemp +=
			'<span class="warn">Вы уверены что хотите удалить эту запись?<br />';
		thisTemp += 'Это действие нельзя отменить!</span>';
		thisTemp += '</td>';
		thisTemp += '</tr>';
		return thisTemp;
	},
	clickAddRowConfirm: function (thisIdentity) {
		...
		$(document).on('click', thisIdentity, function (e) {
			...
			if ($('#clickRemove-' + thisId).length === 0) {
				thisParent.before(
					adminObject.clickRemoveRowTemplate(
						thisId,
						thisSpan,
						thisUrl
					)
	...
	blurUpdateHideShow: function (thisIdentity) {
		...
		$(document).on('focusout', thisIdentity, function (e) {
			...
			if (!adminObject.isEmpty(thisValue)) {
				...
				thisObj.before('<p class="warn">Введите значение</p>');
			}
		});
	},
